...
inline constexpr teller $; // `$` is a GNU identifier extension (accepted by gcc/clang)
} // namespace bank
...
int main() {
  ...
  std::ranges::sort(a, {}, $.balance()); // domain projection
  assert(a[0].balance() == 100 && a[2].balance() == 300);

  $.deposit(50)(a[0]); // bound-arg closure
  assert(a[0].balance() == 150);

  $.deposit(_)(a[1], 25); // a BLANK in a derived placeholder
  assert(a[1].balance() == 275);

  assert(std::ranges::count_if(a, $.has_value()) == 3);
  return 0;
}
